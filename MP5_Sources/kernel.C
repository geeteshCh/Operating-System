...
// A new variable RR scheduler is defined to use the Round Robin scheduler. Comment this if you want to test the code with the simple FIFO scheduler.
...
int main() {
    ...
    // If the RR scheduler is not defined, a simple timer is registered just sends a message that one second has passed every second.
    #ifndef _USES_RR_SCHEDULER_
        SimpleTimer timer(100); /* timer ticks every 10ms. */
    #else
    // If the RR scheduler is defined, an EOQ timer which context switches to new thread after every 50ms is registered.

        EOQTimer timer(100); /* timer ticks every 50ms. */
    #endif
